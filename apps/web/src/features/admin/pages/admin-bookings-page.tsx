...
export default function AdminBookingsPage() {
  ...
  const query = useQuery({
    ...
    queryFn: () =>
      getAdminData<Paginated<AdminBooking>>('/admin/bookings', {
        page,
        limit: 20,
  ...
  const mutation = useMutation({
    mutationFn: () =>
      patchAdminData(`/admin/bookings/${selected?._id}/action`, {
        action,
  ...
  const open = (booking: AdminBooking, next: BookingAction) => {
    setSelected(booking)
    setAction(next)
    setReason('')
    setNotice('')
  ...
  return (
    <div className="admin-page">
      <AdminPageHeader
        ...
        subtitle="Review every platform booking, payment state, schedule, and administrative intervention."
      />
      ...
      <Card className="admin-filter-card">
        <form
          className="admin-filter-grid admin-filter-grid-wide"
          onSubmit={(event) => {
        ...
        >
          <label>
            ...
            <Input
              ...
              placeholder="Student, teacher, topic, or meeting ID"
            />
          </label>
          <label>
            Booking status
      ...
      {query.isLoading ? (
        ...
        <AdminEmpty
          ...
          message="There are no bookings for these filters."
        />
      ) : (
        <Card>
          <AdminTable
          ...
          >
            {query.data.items.map((booking) => {
              const displayStatus = booking.operationalStatus ?? booking.status
              const canCancel = ![
                'completed',
                'cancelled',
                'rejected',
              ...
              return (
                <tr key={booking._id}>
                  <td>
                    <strong>{booking.topicName}</strong>
                    <small>
                      {booking.subjectId?.name ?? 'Subject unavailable'}
                    </small>
                  </td>
                  <td>
                    <strong>{booking.studentId?.name ?? 'Student'}</strong>
                    <small>with {booking.teacherId?.name ?? 'Teacher'}</small>
                  </td>
                  <td>
                  ...
                  <td>
                    <div className="admin-row-actions">
                      ...
                      {booking.canComplete && (
                        <Button
                          onClick={() => open(booking, 'mark-completed')}
                        >
      ...
      {selected && (
        <AdminModal
          title={
            action === 'cancel'
              ? `Cancel ${selected.topicName}`
              : `Complete ${selected.topicName}`
          }
        ...
        >
          ...
          >
            <p className="muted">
              ...
              {selected.studentId?.name} with {selected.teacherId?.name}
            </p>
            <label>
              {action === 'cancel'
                ? 'Cancellation reason'
            ...
            <div className="button-row">
              ...
              <Button
                className={action === 'cancel' ? 'button-danger' : ''}

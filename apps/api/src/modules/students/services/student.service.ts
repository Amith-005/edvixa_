...
class StudentService {
  ...
  async leaderboard(userId: string) {
    const entries = await StudentProfileModel.aggregate<{
      _id: unknown
      userId: unknown
      name: string
      avatar: string | null
      ...
      { $unwind: '$user' },
      { $match: { 'user.role': 'student', 'user.isActive': true, 'user.deletedAt': null } },
      { $sort: { xp: -1, streak: -1, totalPracticeSessions: -1, _id: 1 } },
      { $limit: 100 },
      { $project: { userId: 1, name: '$user.name', avatar: '$user.avatar', gradeLevel: 1, level: 1, xp: 1, streak: 1, totalPracticeSessions: 1, averageScore: 1 } },
    ])
    const ranked = entries.map((entry, index) => ({
      ...
      practiceSessions: Math.max(0, Number(entry.totalPracticeSessions) || 0),
      averageScore: clampPercent(entry.averageScore),
      isCurrentUser: String(entry.userId) === userId,
    }))
    const visibleRank = ranked.find((entry) => entry.isCurrentUser)?.rank ?? null
    const own = visibleRank ? null : await StudentProfileModel.findOne({ userId }).select('xp streak totalPracticeSessions').lean()
    const currentRank = visibleRank ?? (own ? await StudentProfileModel.countDocuments({
      $or: [
        { xp: { $gt: own.xp } },
        { xp: own.xp, streak: { $gt: own.streak } },
        { xp: own.xp, streak: own.streak, totalPracticeSessions: { $gt: own.totalPracticeSessions } },
      ],
    }) + 1 : null)
    return { entries: ranked, currentRank, totalStudents: await StudentProfileModel.countDocuments() }
  }

  async fees(userId: string) {
    const [items, totals] = await Promise.all([
      FeeModel.find({ studentId: userId })
        .populate('teacherId', 'name email avatar')
        .populate('bookingId', 'topicName scheduledAt status subjectId')
        .sort({ createdAt: -1 })
        .limit(100)
        .lean(),
      FeeModel.aggregate<{ paid: number; pending: number; refunded: number }>([
        { $match: { studentId: new Types.ObjectId(userId) } },
        { $group: { _id: null, paid: { $sum: { $cond: [{ $eq: ['$status', 'paid'] }, '$totalAmount', 0] } }, pending: { $sum: { $cond: [{ $eq: ['$status', 'pending'] }, '$totalAmount', 0] } }, refunded: { $sum: '$refundAmount' } } },
      ]),
    ])
    return { items, totals: totals[0] ?? { paid: 0, pending: 0, refunded: 0 } }
  }

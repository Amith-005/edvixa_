...
export async function leaderboard(req: Request, res: Response): Promise<void> {
  res.json({ success: true, data: await studentService.leaderboard(req.user!.id) })
}
export async function fees(req: Request, res: Response): Promise<void> {
  res.json({ success: true, data: await studentService.fees(req.user!.id) })
}

...
class StudentSessionService {
  ...
  async reschedule(
    ...
    const newTimezone = newSlot.timezone ?? booking.timezone ?? 'Asia/Kolkata'
    const newScheduledAt = zonedDateTimeToUtc(newSlot.date, newSlot.startTime, newTimezone)
    const newEndAt = zonedDateTimeToUtc(newSlot.date, newSlot.endTime, newTimezone)
    try {
      await assertBookingWindow(newScheduledAt)
    } catch (error) {
      await AvailabilityModel.updateOne(
        { _id: newSlot._id },
        { $set: { isBooked: false, bookingId: null } },

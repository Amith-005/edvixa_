...
function serializeSlot(slot: AvailabilityLean) {
  const timezone = slot.timezone ?? 'Asia/Kolkata'
  const scheduledAt = zonedDateTimeToUtc(slot.date, slot.startTime, timezone)
  const endAt = zonedDateTimeToUtc(slot.date, slot.endTime, timezone)
...
class TeacherBookingService {
  ...
  async suggestSlot(
    ...
    const timezone = newSlot.timezone ?? booking.timezone ?? 'Asia/Kolkata'
    const scheduledAt = zonedDateTimeToUtc(newSlot.date, newSlot.startTime, timezone)
    const endAt = zonedDateTimeToUtc(newSlot.date, newSlot.endTime, timezone)

    try {
      await assertBookingWindow(scheduledAt)
    } catch (error) {
      await AvailabilityModel.updateOne(
        { _id: newSlot._id, bookingId: booking._id },
        { $set: { isBooked: false, bookingId: null } },

...
import { assertBookingWindow } from './booking-policy.js'
...
class BookingService {
  private async resolveCheckout(studentId: string, input: CheckoutInput) {
    ...
    await this.releaseExpiredReservationForSlot(input.slotId)
    ...
    const slotTimezone = slot.timezone ?? profile.timezone ?? 'Asia/Kolkata'
    const scheduledAt = zonedDateTimeToUtc(slot.date, slot.startTime, slotTimezone)
    const endAt = zonedDateTimeToUtc(slot.date, slot.endTime, slotTimezone)

    const settings = await assertBookingWindow(scheduledAt)
  ...
  private async expirePendingReservation(filter: Record<string, unknown>) {
    const now = new Date()
      ...
      FeeModel.updateOne(
        { bookingId: booking._id, status: 'pending' },
        {
          $set: {
            status: 'failed',
            failedAt: now,
            failureReason: 'Payment reservation expired',
          },
        },
      ),
      AvailabilityModel.updateOne(
  ...
  private async releaseExpiredReservationForSlot(slotId: string) {
    return this.expirePendingReservation({ availabilitySlotId: slotId })
  }
  ...
  async releaseExpiredReservations(limit = 100) {
    ...
    for (const booking of expired) {
      if (await this.expirePendingReservation({ _id: booking._id })) released += 1
    }
    return released
  }
  ...
  private async finalizeCapturedPayment(
    ...
    // Atomically claim only a live reservation. This prevents a late capture
    // from reviving a booking after the slot has been released and rebooked.
    const activeBooking = booking.paymentStatus === 'paid'
      ? booking
    ...
    if (!activeBooking || ['cancelled', 'rejected'].includes(activeBooking.status)) {
      await this.expirePendingReservation({ _id: booking._id })
      Object.assign(fee, paymentFields, {
        refundStatus: 'requested',
        refundAmount: fee.totalAmount,
        refundReason: 'Payment was captured after the reservation expired or was cancelled',
      })
      await fee.save()
    ...
    Object.assign(fee, paymentFields)
    await fee.save()
    return this.buildBookingResult(activeBooking._id)
  }

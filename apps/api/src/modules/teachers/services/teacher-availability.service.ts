...
class TeacherAvailabilityService {
  ...
  private serialize(slot: SerializableAvailability) {
    const subjects = Array.isArray(slot.subjectIds)
      ? slot.subjectIds.map((subject) => ({
          id: String(isRecord(subject) ? subject._id ?? subject : subject),
          name: isRecord(subject) && typeof subject.name === 'string'
            ? subject.name
            : 'Subject',
        }))
      : []

    const bookingRecord = isRecord(slot.bookingId) ? slot.bookingId : null
    const booking = bookingRecord
      ? {
          id: String(bookingRecord._id ?? ''),
          status: typeof bookingRecord.status === 'string' ? bookingRecord.status : null,
          paymentStatus: typeof bookingRecord.paymentStatus === 'string'
            ? bookingRecord.paymentStatus
            : null,
          topicName: typeof bookingRecord.topicName === 'string'
            ? bookingRecord.topicName
            : null,
        }
      : null

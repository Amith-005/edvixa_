...
export async function announcements(req: Request, res: Response): Promise<void> {
  res.json({ success: true, data: await userService.announcements(req.user!.role) })
}
export async function supportTickets(req: Request, res: Response): Promise<void> {
  res.json({ success: true, data: await userService.supportTickets(req.user!.id) })
}
export async function createSupportTicket(req: Request, res: Response): Promise<void> {
  res.status(201).json({ success: true, data: await userService.createSupportTicket(req.user!.id, req.body) })
}
